...
const collegeSchools = [ {"tid": 159, "cid": 0, "did": 0, "school": "Boston College", "name": "Eagles", "abbrev": "BC", "pop": 161, "city": "Chestnut Hill", "state": "MA", "latitude": 42.330, "longitude": -71.166},
...
//These are both the props that I pass into my classes
...
//Here is the school class that makes each card
...
//This functions then lists out all the cards
function SchoolListInfo(){
  return(
    <div>
      <h1>Here is the info for all the schools</h1>
      {collegeSchools.map((oneSchool: SchoolProps) => <School {...oneSchool}/>)}
    </div>
  );
}
...
//This class makes a card for each school name
class SchoolName extends React.Component<SchoolNameProps>{
  render(){
    
    const oneSchoolName = this.props;

    return(
      <div>
        <p>{oneSchoolName.school}</p>
      </div>
...
//This function lists out each school name
function SchoolNameInfo(){
  return(
    <div>
      <h2>Here is a list of all the schools</h2>
      {collegeSchools.map((oneSchoolName: SchoolNameProps) => <SchoolName {...oneSchoolName}/>)}
    </div>
  );
}
...
//This is the welcoming function
function Welcome(){
  return(
    <div>
    <h1>Welcome to the College Basketall Site!</h1>
    <h2>This site lists out all colleges in the NCAA that play basketball</h2>
    </div>
  )
}
...
//This is the app function that is called in the main index page. I didn't delete any default stuff
//I just added my own I will fix it later
function App() {
  return (
    <div className="App">
